...
CREATE TABLE adresses (
	id INTEGER PRIMARY KEY AUTO_INCREMENT,
	contact_id INTEGER,
	type_adresse CHAR(3) NOT NULL,
	adresse VARCHAR(1000) NOT NULL,
	
	CONSTRAINT fk_adresses_contact_id FOREIGN KEY (contact_id) REFERENCES contacts(id) ON DELETE CASCADE,
	CONSTRAINT fk_adresses_types_adresse FOREIGN KEY (type_adresse) REFERENCES types_adresse(code)
);

CREATE TABLE numeros_tel (
	id INTEGER PRIMARY KEY AUTO_INCREMENT,
	contact_id INTEGER,
	type_numero_tel CHAR(3) NOT NULL,
	numero_tel VARCHAR(32) NOT NULL,
	
	CONSTRAINT fk_numeros_tel_contact_id FOREIGN KEY (contact_id) REFERENCES contacts(id) ON DELETE CASCADE,
	CONSTRAINT fk_numeros_tel_type_numero_tell FOREIGN KEY (type_numero_tel) REFERENCES types_numero_tel(code)
);

CREATE TABLE courriels (
	id INTEGER PRIMARY KEY AUTO_INCREMENT,
	contact_id INTEGER,
	type_courriel CHAR(3) NOT NULL,
	courriel VARCHAR(255) NOT NULL,
	
	CONSTRAINT fk_courriels_contact_id FOREIGN KEY (contact_id) REFERENCES contacts(id) ON DELETE CASCADE,
	CONSTRAINT fk_courriels_type_courriel FOREIGN KEY (type_courriel) REFERENCES types_courriel(code)
);
